...
int maxSub_Brute(int A[], int left, int right )
{
	//initialize variables
	...
	cout << "Max sum is: " << max << " and spans A[" << Iindex << ", " << Jindex << "]" << endl;
}
...
int main()
{
	//int A[] = {1, 20, 2, 18, -5, 44};	

	//int A[] = {4,5,-44,-6,10,22};
	int A[] = {1, -30, 20, 20, -3, -2, 18, -30, 5};

	maxSub_Brute(A, 0, 8);
	
	max_subarray ans=find_maximum_subarray(A, 0, 8);
